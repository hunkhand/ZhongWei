...
u32 dwOilOutFifo[10]; 
u32 dwEepromOilValueMax;                                                        //油量数组油量最大值
...
//           ProductPara.OilRange = (ProductPara.OilRange << 8) + temp;
          ...
          dwEepromOilValueMax =  ProductPara.OilRange;   //得到标定的最大油量值。
        }
	...
	SendBuf[j] = UART1_RXBuff[j];
      ...
      /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////   20200511 增加下发油量标定数据后，更新当前滤波数据，不致于延时很久。//////////////
        if(UserParam.FilterLevel == 2)                                     //平滑滤波60s
        {
          for(uint8_t i = 0; i < 60 ; i++)
          {
            memcpy((u8*)(u60sFilter + i), (u8*)(SecFilStr.EverySecCap), 4);
          }
        }
        else if(UserParam.FilterLevel == 3)                                                     //平稳滤波
        {          
           for(uint8_t i = 0; i < UserParam.FilterBufMax ; i++)
          {
               //低滤波值向左FIFO
            memcpy((u8*)(UserParam.LFil + i ) , (u8*)(SecFilStr.EverySecCap), 4);
          }
